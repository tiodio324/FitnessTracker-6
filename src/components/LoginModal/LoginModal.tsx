...
export const LoginModal = observer(() => {
  ...
  const [selectedRole, setSelectedRole] = useState<LoginRole>('trainer');
  const [password, setPassword] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await login(selectedRole, password);
    if (!authStore.loginError) setPassword('');
  };

  const handleClose = () => { closeLoginModal(); setPassword(''); setSelectedRole('trainer'); };

  return (
    <Modal isOpen={loginModalOpen} onClose={handleClose} title="Вход в систему" size="sm">
      <form onSubmit={handleSubmit} className={styles.form}>
        <div className={styles.roleSelector}>
          <button type="button" className={`${styles.roleButton} ${selectedRole === 'trainer' ? styles.active : ''}`} onClick={() => setSelectedRole('trainer')}>
