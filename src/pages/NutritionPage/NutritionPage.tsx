...
export const NutritionPage = observer(() => {
  const { meals, todayMeals, todayCalories, mealsLoading, createMeal, deleteMeal, setFilter, filters } = dataStore;
  ...
  const resetForm = () => { setForm({ name: '', type: 'breakfast', calories: 0, protein: 0, carbs: 0, fat: 0, date: new Date().toISOString().split('T')[0] }); };

  const handleSave = async () => {
    if (!form.name) { uiStore.showError('Введите название приёма пищи'); return; }
    try { await createMeal(form); uiStore.showSuccess('Приём пищи добавлен'); setModalOpen(false); resetForm(); }
  ...
  const columns: TableColumn<Meal>[] = [
    ...
    ...(isTrainer ? [{
      key: 'actions' as keyof Meal, title: '', width: '60px',
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        {isTrainer && <Button variant="primary" onClick={() => { resetForm(); setModalOpen(true); }}>Добавить приём пищи</Button>}
      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Добавить приём пищи"
