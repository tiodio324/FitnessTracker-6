...
const SESSION_EXPIRY_KEY = 'fitness_tracker_session_expiry';
const SESSION_DURATION = 24 * 60 * 60 * 1000;
const AUTH_CREDENTIALS: Record<Exclude<UserRole, 'viewer'>, string> = { trainer: 'trainer2026-fitness', admin: 'admin2026-fitness' };
...
export class AuthStore {
  ...
  get isTrainer(): boolean { return this._user.role === 'trainer' || this._user.role === 'admin'; }
  ...
  canManageWorkouts = (): boolean => this.permissions.canManageWorkouts;
  canManageExercises = (): boolean => this.permissions.canManageExercises;
  canManageMeals = (): boolean => this.permissions.canManageMeals;
  canAccessAdmin = (): boolean => this.permissions.canAccessAdmin;

  hasRole = (r: UserRole): boolean => ({ viewer: 0, trainer: 1, admin: 2 }[this._user.role] >= { viewer: 0, trainer: 1, admin: 2 }[r]);
