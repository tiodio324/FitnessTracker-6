...
function applyDefinedFields<T extends object>(existing: T, patch: object): T {
...
export class DataStore {
  ...
  updateWorkout = async (id: string, data: Partial<WorkoutFormData>): Promise<boolean> => { if (!authStore.canManageWorkouts()) return false; const i = this.workouts.findIndex(w => w.id === id); if (i === -1) return false; const u = applyDefinedFields(this.workouts[i], data); u.updatedAt = new Date().toISOString(); u.exercises = u.exercises ?? []; u.notes = u.notes ?? ''; try { await FirebaseService.setData(`workouts/${id}`, u); runInAction(() => { this.workouts[i] = u; }); return true; } catch { return false; } };
  ...
  updateMeal = async (id: string, data: Partial<MealFormData>): Promise<boolean> => { if (!authStore.canManageMeals()) return false; const i = this.meals.findIndex(m => m.id === id); if (i === -1) return false; const u = applyDefinedFields(this.meals[i], data); u.updatedAt = new Date().toISOString(); try { await FirebaseService.setData(`meals/${id}`, u); runInAction(() => { this.meals[i] = u; }); return true; } catch { return false; } };
